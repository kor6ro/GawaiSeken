    ...
    <AppLayout>
        ...
        <template #header>
            <div class="flex items-center justify-between">
                <h2 class="font-black text-2xl text-foreground leading-tight">Pesan</h2>
                <div class="flex items-center gap-2">
                    <span class="flex h-2 w-2 rounded-full bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.8)] animate-pulse"></span>
                    <span class="text-sm font-semibold text-muted-foreground">{{ props.chats.length }} Chat</span>
                </div>
            </div>
        </template>
            ...
            <div class="max-w-2xl mx-auto sm:px-4">
                ...
                <div class="chat-card">
                    ...
                    <div class="chat-search-bar">
                        <div class="relative">
                            <Search class="absolute left-3.5 top-1/2 -translate-y-1/2 w-[18px] h-[18px] text-muted-foreground/60" />
                            <input
                                v-model="searchQuery"
                                type="text"
                            ...
                            />
                            <button v-if="searchQuery" @click="searchQuery = ''" class="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground transition-colors">
                                <X class="w-4 h-4" />
                            </button>
                        </div>
                    </div>
                    ...
                    <!-- Empty State -->
                    <div v-if="filteredChats.length === 0" class="flex flex-col items-center justify-center py-20 px-8 text-center">
                        <div class="relative mb-6">
                            <div class="absolute inset-0 bg-primary/20 rounded-full blur-3xl opacity-50 scale-150 animate-pulse"></div>
                            <div class="relative bg-muted p-6 rounded-[2rem] shadow-xl border border-border">
                                <MessageSquare class="w-12 h-12 text-primary" />
                            </div>
                        </div>
                        ...
                        <p class="text-muted-foreground text-sm leading-relaxed max-w-xs">
                            {{ searchQuery ? 'Coba kata kunci lain.' : 'Temukan produk dan mulai mengobrol dengan penjual.' }}
                        </p>
                        <Link v-if="!searchQuery" :href="route('home')"
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center justify-between mb-0.5">
                                    <h3 class="chat-item-name"
                                        :class="chat.unread_count > 0 ? 'font-bold' : ''">
                                        {{ getDisplayName(chat) }}
                                    </h3>
                                    <span class="chat-item-time"
                                          :class="chat.unread_count > 0 ? 'text-primary font-semibold' : 'text-muted-foreground'">
                                        {{ formatTime(chat) }}
                                    </span>
                                </div>
                                ...
                                <div class="flex items-center gap-1.5 min-w-0">
                                    ...
                                    <template v-if="isMine(chat)">
                                        <CheckCheck v-if="isRead(chat)" class="w-4 h-4 text-primary flex-shrink-0" />
                                        <CheckCheck v-else class="w-4 h-4 text-muted-foreground/50 flex-shrink-0" />
                                    </template>

                                    <p class="chat-item-preview"
                                       :class="chat.unread_count > 0 ? 'text-foreground font-medium' : 'text-muted-foreground'">
